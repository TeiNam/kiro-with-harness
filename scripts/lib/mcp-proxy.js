...
function ensureMcpProxy({ root, service = 'mcp-proxy', dryRun = false, run = spawnSync, log = console.log } = {}) {
  ...
  // 테스트/CI 탈출구: 실제 설치를 수행하는 e2e 테스트가 호스트의 docker 상태를 바꾸지 않도록
  // 프로비저닝만 건너뛴다(mcp.json 구성은 그대로 검증된다).
  if (process.env.KIRO_HARNESS_SKIP_PROXY_PROVISION) {
  ...
  if (!ver || ver.status !== 0) {
    log(`  ⚠️  Docker 가 설치되어 있지 않습니다 — ${service} 는 Docker 컨테이너로 실행됩니다.`);
    log('      1) Docker 설치 (macOS: Docker Desktop https://docker.com/products/docker-desktop, 또는 `brew install colima docker && colima start`)');
    log(`      2) 설치 후 다시 실행하거나, 직접 기동: \`cd mcp-proxy && docker compose up -d ${service}\``);
    return 'no-docker';
  }

  // 2) docker 데몬 실행 확인 겸 컨테이너 조회 — 데몬 다운이면 "시작하고 다시 실행" 안내
  const ps = run('docker', ['ps', '--filter', `name=^${service}$`, '--format', '{{.Names}}'], { encoding: 'utf8' });
  if (!ps || ps.status !== 0) {
    log('  ⚠️  Docker 는 설치돼 있으나 데몬이 실행 중이 아닙니다(또는 접근 불가).');
    log('      Docker Desktop 을 실행하거나 Colima 를 시작(`colima start`)한 뒤 다시 실행하세요.');
    log(`      Docker 기동 후: cd mcp-proxy && docker compose up -d ${service}`);
  ...
  // 3) dry-run 은 실행하지 않음
  if (dryRun) {
    log(`  DRY-RUN: ${service} 미기동 상태 — \`docker compose up -d ${service}\`(mcp-proxy/) 를 실행할 예정입니다.`);
  ...
  log(`  WARN: ${service} 기동 실패 — 수동 실행하세요: (cd mcp-proxy && docker compose up -d ${service})${detail}`);
  return 'failed';
}
