...
function resolveRecordingDir(options = {}) {
  ...
  if (typeof process.env.HOOK_SESSION_RECORDING_DIR === 'string' && process.env.HOOK_SESSION_RECORDING_DIR.length > 0) {
    return path.resolve(process.env.HOOK_SESSION_RECORDING_DIR);
  }
