...
| Original `hooks/hooks.json` (Claude Code) | Claude Code schema, incompatible with Kiro. Kiro hooks ship as IDE 1.0 v1 JSON (`.kiro/hooks/*.json`) from the tier installer, or embedded in agent JSON for the CLI tier |
...
│   ├── pre-write-guard.json            (PreToolUse/write → agent: size+security+doc location)
│   ├── review-on-stop.json             (Stop → agent: code review)
│   ├── capture-lessons.json            (Stop → agent: propose a repeatable lesson)
│   └── changelog-on-commit.json        (PreToolUse/shell → agent: date-organized CHANGELOG)
