...
// 커스텀 에이전트 관련 OUTDATED 기술을 확정 출처(kiro.dev)에 맞게
// 교체했음을 "예시(example)"로 검증한다. 문서는 2026-07-11 전면 개정으로
// Claude Code / Kiro CLI / Kiro IDE 3자 비교로 확장됐고, 본 테스트는
// 교체 대상 위치에서 다음을 단언한다(형식 비의존, 의미 보존).
...
//   3) 참조 출처 + 확인 일자 기록: kiro.dev 참조와 "2026-07-11"
//
...
test('CONFIRMED: 두 문서 모두 내장 서브에이전트(context gathering, general purpose)를 명시한다 (R3.4)', () => {
  for (const { label, text } of DOCS) {
    // 공식 표기는 공백형("context gathering")이나, 하이픈 식별자형도 허용한다.
    assert.ok(
      /context[-\s]gathering/i.test(text),
      `${label}: 내장 서브에이전트 "context gathering" 기술이 없다`
    );
    assert.ok(
      /general[-\s]purpose/i.test(text),
      `${label}: 내장 서브에이전트 "general purpose" 기술이 없다`
    );
  }
});

test('CONFIRMED: 두 문서 모두 Opus 4.8 행(1M / 128K / 2.2x)을 포함한다 (R3.2)', () => {
  for (const { label, text } of DOCS) {
    // 표시명("Claude Opus 4.8") 또는 식별자("claude-opus-4.8")로 표기된 행을,
    // 1M 컨텍스트를 함께 담은 줄로 찾는다(표 형식 변경에 견고).
    const opusRow = text
      .split('\n')
      .find((line) => /(claude-opus-4\.8|Opus 4\.8)/i.test(line) && line.includes('1M'));
    assert.ok(
      opusRow,
      `${label}: Opus 4.8 과 1M 컨텍스트를 함께 기술한 표 행이 없다`
    );
...
test('REFERENCE: 두 문서 모두 kiro.dev 참조 출처를 기록한다 (R3.5)', () => {
  for (const { label, text } of DOCS) {
    assert.ok(
      /kiro\.dev\//.test(text),
      `${label}: 참조 출처 kiro.dev 가 기록되어 있지 않다`
    );
  }
});

test('REFERENCE: 두 문서 모두 확인 일자 "2026-07-11"을 기록한다 (R3.5)', () => {
  for (const { label, text } of DOCS) {
    assert.ok(
      text.includes('2026-07-11'),
      `${label}: 확인 일자 "2026-07-11"이 기록되어 있지 않다`
    );
  }
});
