...
- **앵커로 구간 찾기(중요)**: finding의 anchor 문자열을 부분 치환 도구의 매칭 대상으로 써서 정확히 그 구간만 수정한다. anchor가 원문에 없거나 여러 번 나오면 해당 finding을 건너뛰고 `unresolved_findings`에 사유와 함께 기록한다.
- **작성 모드**: playbook의 장르별 골격으로 뼈대 → 입력 노트의 사실로 채움 → 처음부터 정확성·코드·구조·용어·전제 충족. 빠진 핵심은 TODO 마커.
- **윤문 모드**: finding 단위 국소 수술 + 구조 보강.
- **문단/절 단위 커밋**: 한 단위를 끝낸 뒤 다음으로. 용어·식별자 일관성 유지.
- **변경률 산정**: 산문 문장 단위 = `(수정·삭제·추가된 산문 문장 수) ÷ (원본 산문 문장 수)`. Do-NOT 구간(코드블록·표) 제외. write 모드는 `n/a`, hybrid는 기존 텍스트 구간에만 적용.

## 입력/출력 프로토콜

### 입력
- 입력 텍스트(원문/노트)
- 탐지 리포트 JSON (polish/hybrid 필수, write 모드는 선택)
- `playbook_path`: 오케스트레이터가 절대경로로 전달 (상대경로 가정 금지)
...
- diff 메타데이터:
  - task_mode
  - char_count_before / after
  - prose_sentences_before / changed
  - change_rate, change_rate_method
  - findings_resolved / unresolved
  - over_polish_warning
  - structure_violation
  - todo_markers
  - edits 배열 (finding_id, category, anchor, before/after, reason)
  - unresolved_findings 배열

## 카테고리별 작업 순서 (윤문 모드 권장)

1. **D(hype·관용구)**: 삭제가 가장 결정적. 먼저 제거하면 문장이 짧아져 후속이 쉬움. 안티테제("X가 아니라 Y다")는 대조 틀을 지우고 긍정 절만 직설로, 메타담화("아시다시피")는 삭제.
2. **A(번역투)**: 조사·어미·어순·수동태를 한국어/자연 영어로 복원.
3. **B(모호성)**: 평가절하·막연어를 구체 수치·조건으로.
4. **H(능동·간결)**: 문두 접속사 제거, "~할 수 있다" 단언화, 시제 통일.
5. **F(용어)**: 표기·약어 일관성 통일.
6. **C(코드)**: 언어 태그·플레이스홀더·인라인 코드·위험 경고 보강. 본문↔코드 불일치는 임의 변경 말고 TODO 플래그. 미완결 스니펫은 import·선언 보강하되 **새 API 날조 금지**. **C-7(실제 비밀정보)**: 플레이스홀더로 치환 — Do-NOT의 유일한 예외로 코드 내부 수정 허용. 노출 비밀엔 "회전 권고" 주석.
7. **E(구조)**: 절차→번호 목록, 비교→표, 헤딩 위계 정상화 (보강). 단어 조각 불릿은 산문으로 병합(과잉 금지), 경고는 콜아웃으로 격상.
8. **G(전제)**: 전제조건 블록·용어 정의 추가.
9. **I(위생)**: 마크다운 교정, 미완성 마커 처리, em-dash는 문맥 부호로 환원(단 `--flag`·`1—10`·코드 내부 대시는 보존). 이미지 alt·불투명 링크 텍스트 교정.

## 에러 핸들링

- **anchor 매칭 실패**: 해당 finding 건너뛰고 `unresolved_findings`에 사유 기록, 오케스트레이터 경고.
- 변경률 60% 초과(**polish 모드 한정**): 작업 중단, 마지막 안정 버전 롤백, `over_polish_warning: true`. write/hybrid의 신규 생성 구간은 트리거하지 않는다.
- 구조 제거 감지: `structure_violation: true` + 해당 edit 롤백.
