...
ANALYSIS_COOLDOWN="${OBSERVER_ANALYSIS_COOLDOWN:-60}"
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping claude analysis on Windows due to known non-interactive hang issue (#295). Set OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
  ...
  MAX_ANALYSIS_LINES="${OBSERVER_MAX_ANALYSIS_LINES:-500}"
  analysis_file="$(mktemp "${TMPDIR:-/tmp}/observer-analysis.XXXXXX.jsonl")"
...
PROMPT
  ...
  timeout_seconds="${OBSERVER_TIMEOUT_SECONDS:-120}"
  max_turns="${OBSERVER_MAX_TURNS:-10}"
  exit_code=0
  ...
  SKIP_OBSERVE=1 HOOK_PROFILE=minimal claude --model haiku --max-turns "$max_turns" --print \
