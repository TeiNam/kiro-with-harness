...
#   - observer recording its own Haiku observer sessions (self-loop)
#   - observer recording other tools' automated sessions
...
# (HOOK_PROFILE=minimal, SKIP_OBSERVE=1, agent_id, path exclusions).
...
[ "${HOOK_PROFILE:-standard}" = "minimal" ] && exit 0

# Layer 3: cooperative skip env var for automated sessions.
[ "${SKIP_OBSERVE:-0}" = "1" ] && exit 0
...
SIGNAL_EVERY_N="${OBSERVER_SIGNAL_EVERY_N:-20}"
