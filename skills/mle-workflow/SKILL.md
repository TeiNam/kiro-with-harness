...
- `clickhouse-io` and `duckdb-patterns` for data storage and analytics surfaces
...
| `clickhouse-io` / `duckdb-patterns` | Version labels, feature snapshots, prediction logs, experiment metrics, and drift analytics |
...
| MLE-02 | Define metric goals, labels, data sources, and the mistake budget | `repo-scan`, `database-reviewer`, `clickhouse-io` | Data and metric contract with entity grain, label timing, label confidence, feature timing, point-in-time joins, split policy, and dataset snapshot | data contract, metric design, leakage, reproducibility |
